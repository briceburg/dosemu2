 ...
 *  MA 02110-1301, USA.
 *
 *  Ported to DJGPP by Bart Oldeman, 2006-2009, but still runs under Linux
 *  too, of course.
 *  See comments for differences and current issues.
...
void test_segs(void)
{
...
#ifdef __DJGPP__
    {
      ...
      base = dsbase + (unsigned long)&seg_data1;
      limit = (sizeof(seg_data1) + 0xfff) >> 12;
      ...
      buf[3] = (base >> 8)&0xff;
      buf[4] = (base >> 16)&0xff;
      buf[6] |= 0xc0; //32-bit, limit in pages
      buf[7] = (base >> 24)&0xff;
      __dpmi_set_descriptor(MK_SEL(1), &buf);

      __dpmi_allocate_specific_ldt_descriptor(MK_SEL(2));
      __dpmi_get_descriptor(MK_SEL(2), &buf);
      base = dsbase + (unsigned long)&seg_data2;
      limit = (sizeof(seg_data2) + 0xfff) >> 12;
      buf[0] = limit & 0xff;
      buf[1] = (limit >> 8) & 0xff;
      buf[2] = base & 0xff;
      buf[3] = (base >> 8)&0xff;
      ...
      buf[6] |= 0xc0; //32-bit, limit in pages
      buf[7] = (base >> 24)&0xff;
      __dpmi_set_descriptor(MK_SEL(2), &buf);
    }
#endif
...
/* 16 bit code test */
extern char code16_start asm("code16_start"), code16_end asm("code16_end");
extern char code16_func1 asm("code16_func1");
extern char code16_func2 asm("code16_func2");
extern char code16_func3 asm("code16_func3");
...
void test_code16(void)
{
    ...
    buf[5] |= 0x8; //code
    buf[6] &= ~0xc0;
    buf[6] |= 0x00; //16-bit, limit in bytes
    buf[7] = (base >> 24)&0xff;
    __dpmi_set_descriptor(MK_SEL(1), &buf);
#endif
...
extern char func_lret asm ("func_lret");
extern char func_iret asm ("func_iret");
...
extern char vm86_code_start asm("vm86_code_start");
extern char vm86_code_end asm("vm86_code_end");
...
static char buf[1024], *bufptr = buf;
void int21_handler(_go32_dpmi_registers *r)
{
  int ah, v;
  ah = (r->d.eax >> 8) & 0xff;
  switch(ah) {
  ...
  case 0x09: /* write string */
    {
      uint8_t c, *ptr;
      ptr = seg_to_linear(r->x.ds, r->d.edx);
      for(;;) {
	c = _farpeekb(_dos_ds,(unsigned)ptr++);
	if (c == '$')
	  break;
	*bufptr++ = c;
      }
      r->d.eax = (r->d.eax & ~0xff) | '$';
    }
    break;
...
#define VIF_MASK        0x00080000
 ...
 the_end:
    ...
    memset(&r, 0, sizeof(r));
    seg = __dpmi_allocate_dos_memory(
			       (&vm86_code_end - &vm86_code_start + 0x10f) >> 4,
                               &sel);
    r.x.ip = VM86_CODE_IP;
    r.x.sp = 0xfffe;
    r.x.cs = seg;
    ...
    _go32_dpmi_get_real_mode_interrupt_vector(0x21, &old_vector);
    info.pm_offset = (unsigned)int21_handler;
    _go32_dpmi_allocate_real_mode_callback_iret(&info, &regs);
    _go32_dpmi_set_real_mode_interrupt_vector(0x21, &info);

    __dpmi_simulate_real_mode_procedure_retf(&r);

    _go32_dpmi_set_real_mode_interrupt_vector(0x21, &old_vector);
...
#else
    act.sa_handler = sig_trap_handler;
#endif
    sigemptyset(&act.sa_mask);
#ifdef SA_SIGINFO
...
extern int smc_code2(int) asm("smc_code2");
